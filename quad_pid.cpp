...
float current_y;
float current_z;
void obsCallback(const geometry_msgs::Pose2D::ConstPtr& msg);
...
int main(int argc, char **argv)
{
  ...
  theta = 0;
  ros::NodeHandle nh;
  ros::Subscriber quadpose1_sub = nh.subscribe("/ground_truth/state", 100, quadCallback); // subscriber to get MAV position
  ros::Subscriber quadpose_sub = nh.subscribe("mavros/local_position/odom", 10, feedbackfn);
  ros::Subscriber obspose_sub = nh.subscribe("lidar", 100, obsCallback); // subscriber to get ground bot position
...
void obsCallback(const geometry_msgs::Pose2D::ConstPtr& msg)
{
  obspose.pose.pose.position.x = (msg->x)*(cos((msg->theta/180)*3.14))+current_x;
  obspose.pose.pose.position.y = (msg->x)*(sin((msg->theta/180)*3.14))+current_y;
  obspose.pose.pose.position.z = Default;
  return;
}
void quadCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
